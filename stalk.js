class Stalk {
  constructor(posX, posY) {
    ...
    this.weight = WEIGHT;
    this.numTris = random(5);
    this.height = this.up ? min(this.posY, random(50, 100)) : min(height-this.posY, random(50, 100));
    this.dim = random(5, this.height/this.numTris*0.8);
    ...
    this.nextPosY = this.posY + (this.up ? -1 : 1)*(this.height);
  }
  ...
  render() {
    ...
    push();
      ...
      push();
        translate(0, this.up ? -this.height : this.height);
        for (let i = 0; i < this.numTris; i++) {
